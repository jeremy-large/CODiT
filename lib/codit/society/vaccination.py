...
def vaccinate(people, proportion, immune_response=ImmuneResponse.ASTRAZENECA_1ST_DOSE):
    to_vaccinate = int(np.round(len(people) * proportion))
    for p in random.sample(people, to_vaccinate):
        p.vaccinate_with(immune_response)
...
def vaccinate_per_table(people, efficacy, immune_response=ImmuneResponse.ASTRAZENECA_1ST_DOSE):
    ...
    logging.info(f"Vaccinating {immune_response.name} across MSOAs according to tabulation, at an efficacy of {efficacy}")
    for msoa in msoas(people):
        ...
        for min_age, max_age, desc in ages:
            ...
            vaccinate(residents, percent * 0.01 * efficacy, immune_response=immune_response)
